...
resourcestring
  sInvalidDir = 'Diretório inválido: ';
...
procedure Register;
begin
  RegisterComponents('DT Inovacao', [TDTDirectory]);
end;
//----------------------------------------------------------------------------
...
constructor TDTDirectory.Create(aOwner: TComponent);
begin
...
destructor TDTDirectory.Destroy;
begin
  Active := false;
  if not (csDesigning in ComponentState) then
...
procedure TDTDirectory.WndProc(var aMsg: TMessage);
begin
  with aMsg do
...
procedure TDTDirectory.SetNotifyFilters(aNotifyFilters: TNotifyFilters);
begin
...
procedure TDTDirectory.SetWatchSubDirs(aWatchSubDirs: boolean);
begin
...
procedure TDTDirectory.SetDirectory(aDir: string);
begin
  if aDir = '' then
  begin
    Active := false;
...
procedure TDTDirectory.SetActive(aActive: boolean);
var
  nf: dword;
begin
